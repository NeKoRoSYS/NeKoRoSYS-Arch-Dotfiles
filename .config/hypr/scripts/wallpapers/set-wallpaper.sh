...
if [[ "$SELECTED_FILE" =~ ^http ]]; then
    ...
    case "$(echo "$EXT" | tr '[:upper:]' '[:lower:]')" in
	    ...
	    if ~/.config/hypr/scripts/notify-download.sh "$SELECTED_FILE" "$DEST"; then
            ...
            ;;
        *)
	    notify-send -a "Download" -u critical \
			-h string:x-canonical-private-synchronous:download \
			"Error" "URL does not point to a valid media extension (png, jpg, jpeg, mp4, mkv, webm)."
            exit 1
            ;;
    esac
fi
